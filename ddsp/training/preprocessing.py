...
class Preprocessor(object):
  ...
  def __call__(self, features, training=True):
    ...
    """
    return core.copy_if_tf_function(features)
...
class DefaultPreprocessor(Preprocessor):
  ...
  def _default_processing(self, features):
    ...
    for k in ['loudness_dbM', 'loudness_dbL', 'loudness_dbR', 'f0_hzM', 'f0_hzL', 'f0_hzR']:
      features[k] = at_least_3d(features[k])
      features[k] = core.resample(features[k], n_timesteps=self.time_steps)
    # For NN training, scale frequency and loudness to the range [0, 1].
    # Log-scale f0 features. Loudness from [-1, 0] to [1, 0].
    features['f0_scaledM'] = hz_to_midi(features['f0_hzM']) / F0_RANGE
    features['f0_scaledL'] = hz_to_midi(features['f0_hzL']) / F0_RANGE
    features['f0_scaledR'] = hz_to_midi(features['f0_hzR']) / F0_RANGE
    features['ld_scaledM'] = (features['loudness_dbM'] / LD_RANGE) + 1.0
    features['ld_scaledL'] = (features['loudness_dbL'] / LD_RANGE) + 1.0
    features['ld_scaledR'] = (features['loudness_dbR'] / LD_RANGE) + 1.0
    features['f0_scaled'] = features['f0_scaledM']
    features['ld_scaled'] = features['ld_scaledM']
    return features
